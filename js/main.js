...
Run.prototype.createCharacter = function(mainName) {
    ...
    addEvent(this.oParent,'touchmove',function(event){event.preventDefault();});
  	addEvent(this.oParent,'touchend',this.leap(this.character));
    //检测碰撞
    this.checkAll();
}
...
Run.prototype.createBarry = function() {
    ...
    //障碍物颜色
    var barrColor = ['#f88dab', '#8db5f8', '#83e998', '#fff775'];
	  var timer = function creaBarry(ele) {
            ...
            oBarry.style.backgroundColor = barrColor[that.rnd(0, 5)];
            that.oParent.appendChild(oBarry); 
            //移除屏幕外的地标
            oBarry.addEventListener("webkitAnimationEnd", function(){ 
                oBarry.parentNode.removeChild(oBarry);
            }, false);
        }

    var createTime = this.rnd(2500,3000);
    setInterval(timer,createTime);      
}
...
Run.prototype.createLandmark = function (){
    ...
    function _newLM(that){
       return function(){
         that.newLM();
         that.raTime = that.rnd(2000,1500);
